...
\PARstart{I}{n} this study, the analysis of the dataset focuses on examining how people visit and interact with these establishments over time. 
...
\subsection{Time Series Data and Classification}
Time Series Data also known as Temporal Data is a sequence of data points indexed in time order. Time series data is used in various fields such as finance, economics, weather forecasting, and many others. Time Series Classification (TSC) is the task of predicting the class label of a time series. The time series data can be univariate or multivariate. In univariate time series data, there is only one variable, whereas in multivariate time series data, there are multiple variables. Time series data can be classified into two categories: fixed length and variable length. In fixed length time series data, the length of the time series is fixed, whereas in variable length time series data, the length of the time series is not fixed.

\subsection{Urban computing}
Urban computing is a process of acquisition, integration, and analysis of big and heterogeneous data generated by a diversity of sources in urban spaces, such as sensors, devices, vehicles, buildings, and human, to tackle the major issues that cities face, e.g. air pollution, increased energy consumption and traffic congestion. The ultimate goal of urban computing is to improve the quality of life in urban settings. Urban computing integrates big data, cloud computing, Internet of Things, and mobile computing to enhance the intelligence and efficiency of urban systems, infrastructure, and services. Urban computing is a multidisciplinary field that involves experts in computer science, civil engineering, transportation, environmental science, sociology, and urban planning.
