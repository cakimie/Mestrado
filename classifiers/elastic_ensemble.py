...
def run_elastic_ensemble(
    ...
    if clearML:
        if task==None:
            task = Task.init(project_name='PopularTimesFold/Classifier', task_name="elastic_ensemble")
        task.connect(params)
    ...
    main_time = time.time()
    results = elastic_ensemble(X_train, y_train, X_test, y_test)
    if clearML:
